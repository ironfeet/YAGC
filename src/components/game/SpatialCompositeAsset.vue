...
import type { SpatialToy, SpatialVehicle, SpatialPreposition, SpatialOrientation } from '../../types';
...
const toyTransform = computed(() => {
  ...
  if (props.preposition === 'behind') {
    return 'translate(-20, -30) scale(0.7)';
  } else if (props.preposition === 'in_front') {
    return 'translate(30, 40) scale(0.85)';
  } else if (props.preposition === 'inside') {
  ...
  } else if (props.preposition === 'on_top') {
    return 'translate(40, -60) scale(0.6)';
  } else if (props.preposition === 'under') {
    return 'translate(40, 65) scale(0.6)';
  }
...
<template>
  <svg viewBox="0 0 200 200" class="spatial-asset" xmlns="http://www.w3.org/2000/svg">
    ...
    <template v-if="isHeaderOnly && headerType === 'toy' && toy">
      <use :href="`#toy-${toy}`" :transform="toyTransform" />
    </template>
    
    <template v-else-if="isHeaderOnly && headerType === 'vehicle' && vehicle">
      <use :href="`#${vehicle}-back`" :transform="vehicleTransform" />
      <use :href="`#${vehicle}-front`" :transform="vehicleTransform" />
    </template>
    
    <!-- COMPOSITE LOGIC -->
    <template v-else-if="toy && vehicle">
      <!-- BEHIND -->
      <template v-if="preposition === 'behind'">
        <use :href="`#toy-${toy}`" :transform="toyTransform" />
        <use :href="`#${vehicle}-back`" :transform="vehicleTransform" />
        <use :href="`#${vehicle}-front`" :transform="vehicleTransform" />
      </template>
      ...
      <!-- IN FRONT -->
      <template v-else-if="preposition === 'in_front'">
        <use :href="`#${vehicle}-back`" :transform="vehicleTransform" />
        <use :href="`#${vehicle}-front`" :transform="vehicleTransform" />
        <use :href="`#toy-${toy}`" :transform="toyTransform" />
      </template>
      ...
      <!-- UNDER -->
      <template v-else-if="preposition === 'under'">
        <use :href="`#toy-${toy}`" :transform="toyTransform" />
        <use :href="`#${vehicle}-back`" :transform="vehicleTransform" />
        <use :href="`#${vehicle}-front`" :transform="vehicleTransform" />
      </template>
      ...
      <!-- ON TOP -->
      <template v-else-if="preposition === 'on_top'">
        <use :href="`#${vehicle}-back`" :transform="vehicleTransform" />
        <use :href="`#${vehicle}-front`" :transform="vehicleTransform" />
        <use :href="`#toy-${toy}`" :transform="toyTransform" />
      </template>
