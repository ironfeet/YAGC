...
import PerspectiveCompositeAsset from '../../../components/game/PerspectiveCompositeAsset.vue';
import PointingHand from '../../../components/prompts/PointingHand.vue';
import type { PerspectiveTakingLevelConfig, PerspectiveItemConfig, SpatialToy, SpatialVehicle, PerspectivePreposition, SpatialOrientation } from '../../../types';

const moduleId = 'tier3-perspectivetaking';
...
const config = ref<PerspectiveTakingLevelConfig | null>(null);
const options = ref<{ item: PerspectiveItemConfig, isTarget: boolean }[]>([]);
...
const PREPOSITIONS: PerspectivePreposition[] = ['left', 'inside', 'right'];
...
const getRuleText = (rule: PerspectivePreposition) => rule === 'inside' ? 'inside' : `on the ${rule}`;
...
const generateLevel = () => {
  ...
  const selectedColors = shuffle([...COLORS]).slice(0, gridRows);
  
  const rowHeaders = selectedToys.map((t, i) => ({ toy: t, color: selectedColors[i] }));
  ...
  const ruleMapping: Record<string, PerspectivePreposition> = {};
  let audioPrompt = '';

  if (phase === 1 || phase === 2) {
    const globalRule = getRandomItem(PREPOSITIONS);
    rowHeaders.forEach(rh => ruleMapping[rh.color] = globalRule);
    audioPrompt = `Put the toys ${getRuleText(globalRule)}.`;
  } else {
    // Phase 3, 4, 5: Complex mixed rules based on colors
    const rule1 = getRandomItem(PREPOSITIONS);
    const remaining = PREPOSITIONS.filter(p => p !== rule1);
    const rule2 = getRandomItem(remaining);
    
    ruleMapping[rowHeaders[0].color] = rule1;
    ruleMapping[rowHeaders[1].color] = rule2;
    
    if (gridRows > 2 && rowHeaders[2]) {
      const rule3 = getRandomItem(PREPOSITIONS.filter(p => p !== rule1 && p !== rule2));
      ruleMapping[rowHeaders[2].color] = rule3;
      audioPrompt = `Put the ${COLOR_NAMES[rowHeaders[0].color]} toys ${getRuleText(rule1)}, the ${COLOR_NAMES[rowHeaders[1].color]} toys ${getRuleText(rule2)}, and the ${COLOR_NAMES[rowHeaders[2].color]} toys ${getRuleText(rule3)}.`;
    } else {
      audioPrompt = `Put the ${COLOR_NAMES[rowHeaders[0].color]} toys ${getRuleText(rule1)}, and the ${COLOR_NAMES[rowHeaders[1].color]} toys ${getRuleText(rule2)}.`;
    }
  }
  ...
  if (phase === 1) {
    ...
    // Rely on audio. Fill entire grid.
  ...
  const targets: PerspectiveItemConfig[] = emptyCells.map(cell => {
    return {
      ...
      preposition: ruleMapping[rowHeaders[cell.row - 1].color]
    };
  });

  const distractors: PerspectiveItemConfig[] = [];
  const distractorCount = phase === 1 ? 2 : (phase === 2 ? 3 : 4);
  for(let i=0; i<distractorCount; i++){
    let dist: PerspectiveItemConfig = {
      id: `distractor-${i}`,
      toy: getRandomItem(TOYS),
      toyColor: getRandomItem(selectedColors),
      vehicle: getRandomItem(selectedVehicles),
      vehicleColor: getRandomItem(COLORS),
      vehicleOrientation: phase > 1 ? getRandomItem(ORIENTATIONS) : 'left',
      preposition: getRandomItem(PREPOSITIONS)
    };
    if (phase === 5) {
      ...
      else dist.preposition = getRandomItem(PREPOSITIONS.filter(p => p !== ruleMapping[baseTarget.toyColor]));
    }
  ...
  config.value = {
    ...
    instructionText: phase === 1 ? `Put the toys ${getRuleText(ruleMapping[rowHeaders[0].color])}.` : 'Listen to the instructions!',
    phase,
    gridRows,
    gridCols,
    audioPrompt,
...
const getRuleForKey = (r: number) => {
  if (!config.value) return 'inside';
  return config.value.ruleMapping[config.value.rowHeaders[r - 1].color];
};
...
const handleError = () => {
  ...
  registerError();
  playInstruction('Try again! Listen to the rule.');
};
...
<template>
  <div class="perspective-module">
    <!-- START SCREEN -->
    <div v-if="!hasStarted" class="start-screen">
      <div class="start-icon"><MenuIcon :gameId="moduleId" style="width: 140px; height: 140px;" /></div>
      <h1>Perspective<br><span>Taking</span></h1>
      <p class="start-sub">Look closely at the key, and put the toys in their places based on their colors!</p>
    ...
    <div v-else-if="config" class="game-board">
      ...
      <header class="top-bar">
        ...
        <button class="replay-btn" @click="playAudioPrompt">
      ...
      <div class="top-matrix" :class="{ 'matrix-complete': isComplete }">
        <div class="grid-container" :style="{ gridTemplateColumns: `auto repeat(${config.gridCols}, 1fr)` }">
          ...
          <div class="matrix-cell header-empty">
            <div v-if="config.phase === 1" class="rule-key-label">KEY</div>
            <!-- Empty in Phase 2/3 forcing audio reliance -->
          </div>
          
          <!-- COL HEADERS -->
          <div v-for="(col, cIdx) in config.colHeaders" :key="'col'+cIdx" class="matrix-cell col-header">
            <PerspectiveCompositeAsset 
          ...
          <template v-for="(row, rIdx) in config.rowHeaders" :key="'row'+rIdx">
            ...
            <template v-for="(col, cIdx) in config.colHeaders" :key="'cell-'+rIdx+'-'+cIdx">
              <div 
                ...
                :class="{ 
                  'drop-zone perspective-target': isCellEmpty(rIdx + 1, cIdx + 1) && !isCellFilled(rIdx + 1, cIdx + 1),
                  'filled-zone': isCellFilled(rIdx + 1, cIdx + 1) || (!isCellEmpty(rIdx + 1, cIdx + 1) && !(rIdx === 0 && cIdx === 0 && config.phase === 1)),
                  'rule-key-cell': rIdx === 0 && cIdx === 0 && config.phase === 1
                }"
              ...
              >
                ...
                <template v-else>
                  <PerspectiveCompositeAsset 
                    :toy="row.toy" :toyColor="row.color"
                    :vehicle="col.vehicle" :vehicleColor="col.color" :vehicleOrientation="col.orientation"
                    :preposition="getRuleForKey(rIdx + 1)"
                  />
                </template>
              </div>
            </template>
        ...
        <div class="options-container">
          <div class="piece-wrapper" v-for="opt in options" :key="`${levelCounter}-${opt.item.id}`">
            <PuzzlePiece 
              :id="opt.item.id"
              :transparent="true"
              dropZoneSelector=".perspective-target"
