...
import type { 
  TimePrepositionsMemoryLevelConfig, 
  TemporalSequenceRule, 
  TemporalSyntaxType,
  VocabularyNoun
} from '../../../types';

const moduleId = 'tier3-timeprepositions-memory';
...
// ── State ─────────────────────────────────────────────────────────────────────
const config = ref<TimePrepositionsMemoryLevelConfig | null>(null);
...
// Store the initial untouched options for resetting upon error
...
const handleStrictError = (itemId: string) => {
  ...
  // RESET LOGIC
  // Clear the placed items and restore initial options
  placedItems.value = [];
  config.value.options = [...initialOptions.value];
  expectedActionQueue.value = [...config.value.rule.correctOrder];
  
  // Audio playback will hide the board due to isPlaying binding
  playInstruction(`Oops. Listen closely and try to remember: ${config.value.instructionText}`);
};
...
const replayAudio = () => {
  ...
  // If they replay manually, we should also reset the sequence so they have a fresh start
  placedItems.value = [];
  config.value.options = [...initialOptions.value];
  expectedActionQueue.value = [...config.value.rule.correctOrder];
...
<template>
  <div class="time-memory-module">

    <!-- ── START SCREEN ──────────────────────────────────────────────────── -->
    <div v-if="!hasStarted" class="start-screen">
      <div class="start-icon"><MenuIcon :gameId="moduleId" style="width: 140px; height: 140px;" /></div>
      <h1>Time Prepositions<br><span>Memory Training</span></h1>
      <p class="start-sub">Listen carefully. The items will be hidden while you listen. Build the chronological sequence from memory!</p>
    ...
    <div v-else-if="config" class="game-board">
      ...
      <header class="top-bar">
        ...
        <div class="instruction-text">Build the sequence from memory!</div>
