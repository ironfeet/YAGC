...
import type { 
  SubjectObjectLevelConfig, 
  HoseLogicRule, 
  SubjectObjectSyntaxType,
  VocabularyNoun
} from '../../../types';

const moduleId = 'tier3-subjectobject';
const progressStore = useProgressStore();
const gameStore = useGameStore();
const { playInstruction, stopSpeech } = useSpeech();
...
// ── State ─────────────────────────────────────────────────────────────────────
const config = ref<SubjectObjectLevelConfig | null>(null);
...
const validateDone = () => {
  ...
  if (JSON.stringify(selectedIds) === JSON.stringify(targetIds)) {
    ...
    spawnConfetti();
    log.success('subject-object-complete', { phase: config.value.phase });
    ...
    // Spring animals back to yard
    availableAnimals.value = [...availableAnimals.value, ...puddleAnimals.value];
    puddleAnimals.value = [];
    
    playInstruction(`Oops. Listen closely: ${config.value.instructionText}`);
  }
};
...
<template>
  <div class="subject-object-module">
    ...
    <div v-if="!hasStarted" class="start-screen">
      ...
      <h1>Garden Hose Game<br><span>Subject/Object</span></h1>
      <p class="start-sub">Listen to the story and drag the wet animals into the puddle!</p>
