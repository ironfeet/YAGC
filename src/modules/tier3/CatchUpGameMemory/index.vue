...
import type { 
  CatchUpMemoryLevelConfig, 
  CatchUpRaceRule, 
  CatchUpSyntaxType,
  VocabularyNoun
} from '../../../types';

const moduleId = 'tier3-catchup-memory';
...
// ── State ─────────────────────────────────────────────────────────────────────
const config = ref<CatchUpMemoryLevelConfig | null>(null);
...
const handleStrictSuccess = (itemId: string) => {
  ...
  spawnConfetti();
  log.success('catchup-memory-complete', { phase: config.value.phase });
...
const handleStrictError = (itemId: string) => {
  ...
  // RESET LOGIC (Memory Task)
  // Restore initial options and clear podium
  placedWinner.value = null;
  config.value.options = [...initialOptions.value];
  
  // No negative buzzer. Immediately invoke instruction, which toggles isPlaying and hides the board
...
const replayAudio = () => {
  ...
  // Reset sequence on manual replay for memory purity
  placedWinner.value = null;
  config.value.options = [...initialOptions.value];
...
<template>
  <div class="catchup-memory-module">
    ...
    <div v-if="!hasStarted" class="start-screen">
      ...
      <h1>Catch Up Game<br><span>Memory Training</span></h1>
      <p class="start-sub">Listen carefully. The items will be hidden while you listen. Drag the winner to the podium from memory!</p>
    ...
    <div v-else-if="isPlaying" class="focus-listening-mode">
      <div class="listening-content">
        <div class="speaker-pulse-wrapper">
