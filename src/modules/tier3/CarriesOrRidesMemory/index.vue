...
import type { CarriesOrRidesMemoryLevelConfig, AnimalFeature, AnimalShape, AnimalSize, TransportVerb } from '../../../types';

const moduleId = 'tier3-carriesorrides-memory';
...
const config = ref<CarriesOrRidesMemoryLevelConfig | null>(null);
...
const generateLevel = () => {
  ...
  config.value = {
    ...
    distractors: allAnimals.filter(a => a.id.startsWith('distractor')).map(a => ({ shape: a.shape, size: a.size }))
  };
...
const handleSuccess = (draggedId: string, target?: HTMLElement) => {
  ...
  if (draggedId === 'rider' && targetId === 'carrier') {
    ...
    log.success('transport-match-memory', { phase: config.value.phase });
...
const handleError = () => {
  if (isSuccess.value || !config.value) return;
  log.error('wrong-transport-memory', { phase: config.value.phase, promptLevel: currentLevel.value });
...
const playHint = () => {
  if (config.value && !isPlaying.value) {
    playInstruction(config.value.instructionText);
  }
};
</script>
...
<template>
  <div class="carries-or-rides-memory-module">
    <div v-if="!hasStarted" class="start-screen">
      <div class="start-icon"><MenuIcon :gameId="moduleId" style="width: 140px; height: 140px;" /></div>
      <h1>Carries or Rides<br><span>Memory Training</span></h1>
      <p class="start-sub">Listen closely and remember who is carrying and who is riding. Wait until the instruction finishes!</p>
    ...
    <div v-else-if="config" class="game-board">
      <div class="action-layout">
        <header class="top-bar">
          ...
          <button class="replay-btn" :disabled="isPlaying" @click="playHint" :class="{ 'disabled': isPlaying }">
        ...
        <div class="main-area">
          <transition name="fade" mode="out-in">
            <!-- Listening Visual (When isPlayingAudio is true) -->
            <div v-if="isPlaying && !isSuccess" class="listening-overlay">
              <div class="pulsing-speaker">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/>
                  <path d="M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07"/>
                </svg>
              </div>
              <h2 class="listening-text">Listening...</h2>
            </div>
            
            <!-- Success Animation -->
            <div v-else-if="isSuccess" class="success-animation-container">
            ...
            <!-- Interaction Zone (When audio finishes) -->
...
<style scoped>
.carries-or-rides-memory-module {
  width: 100%;
  height: 100%;
  display: flex;
...
.replay-btn.disabled { opacity: 0.5; pointer-events: none; }
...
@keyframes pulse {
  ...
  70% { transform: scale(1.05); box-shadow: 0 0 0 30px rgba(59, 130, 246, 0); }
