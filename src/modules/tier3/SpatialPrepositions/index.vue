...
import type { SpatialPrepositionsLevelConfig, SpatialItemConfig, SpatialToy, SpatialVehicle, SpatialPreposition, SpatialOrientation } from '../../../types';

const moduleId = 'tier3-spatial-prepositions';
...
const config = ref<SpatialPrepositionsLevelConfig | null>(null);
...
const PREPOSITIONS: SpatialPreposition[] = ['in_front', 'inside', 'behind'];
...
const generateLevel = () => {
  ...
  if (phase === 1) {
    ...
    // Phases 4 and 5 (Model Cell / Expert)
  ...
  for(let i=0; i<distractorCount; i++){
    ...
    if (phase === 5) {
      // Very subtle distractors, pick a random target and change just one aspect
