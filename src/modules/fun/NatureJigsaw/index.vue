...
import ColorfulNature from './ColorfulNature.vue';
...
const moduleId = 'fun-nature-jigsaw';
...
const playQueue = ref<NatureDef[]>([]);

const handleStart = () => {
  hasStarted.value = true;
  playQueue.value = shuffleArray([...NATURE_ITEMS]);
  generateLevel();
};

// ── Animal pool ──────────────────────────────────────────────────────────────
type NatureDef = { id: string; label: string; emoji: string; };

const NATURE_ITEMS: NatureDef[] = [
...
const nature = computed(() => playQueue.value[currentNatureIdx.value]);
...
function generateLevel() {
  ...
  log.generate({ level: level.value, natureId: nature.value.id, phase, pieces: rows.value * cols.value });
  safeSetTimeout(() => {
...
function onLevelComplete() {
  if (gameStore.isRandomMode) { setTimeout(() => { if (!gameStore.advanceRandomRound()) nextNature(); }, 2500); } else { completed.value = true; }
  score.value += 10 * level.value;
  progressStore.updateStats(moduleId, true);
}

function nextNature() {
  currentNatureIdx.value++;
  if (currentNatureIdx.value >= playQueue.value.length) {
    // Reshuffle when we run out
    playQueue.value = shuffleArray([...NATURE_ITEMS]);
    currentNatureIdx.value = 0;
  }
  level.value++;
  completed.value = false;
  generateLevel();
...
<template>
  ...
  >
    <div v-if="!hasStarted" class="start-screen">
      <div class="start-icon"><MenuIcon gameId="fun-nature-jigsaw" style="width: 140px; height: 140px;" /></div>
      <h1>Nature Jigsaw<br><span>Spatial Puzzles</span></h1>
      <p class="start-sub">Put the nature pieces together.</p>
      ...
      <div class="action-layout">
        ...
        <header class="top-bar">
          <div class="phase-badge">
            <span style="font-size: 1.5rem; margin-right: 0.5rem">{{ nature.emoji }}</span>
            Level {{ level }} — {{ nature.label }}
          </div>
        ...
        >
          ...
          >
            <g v-if="isComplexScene">
              ...
              <g transform="translate(-10, 10) scale(0.6)"><ColorfulNature :name="playQueue[0]?.id" width="100" height="100" /></g>
              <g transform="translate(45, -5) scale(0.55)"><ColorfulNature :name="playQueue[1]?.id" width="100" height="100" /></g>
              <g transform="translate(25, 45) scale(0.7)"><ColorfulNature :name="playQueue[2]?.id" width="100" height="100" /></g>
              <g transform="translate(-20, 60) scale(0.4)"><ColorfulNature :name="playQueue[3]?.id" width="100" height="100" /></g>
              <g transform="translate(65, 55) scale(0.5)"><ColorfulNature :name="playQueue[4]?.id" width="100" height="100" /></g>
            </g>
            <ColorfulNature v-else :name="nature.id" width="100" height="100" />
          </svg>
    ...
    <transition name="pop">
      <div v-if="completed" class="complete-overlay">
        <div class="complete-card">
          <div class="complete-emoji">{{ nature.emoji }}</div>
          <h2 class="complete-title">{{ nature.label }}!</h2>
          <p class="complete-sub">Great job! You built it! 🎉</p>
          <div class="complete-actions">
            <button class="btn-next" @click="nextNature">Next Nature →</button>
