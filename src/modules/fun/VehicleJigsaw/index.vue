...
import ColorfulVehicle from './ColorfulVehicle.vue';
...
const playQueue = ref<VehicleDef[]>([]);

const handleStart = () => {
  hasStarted.value = true;
  playQueue.value = shuffleArray([...VEHICLES]);
  generateLevel();
};

// ── Animal pool ──────────────────────────────────────────────────────────────
type VehicleDef = { id: string; label: string; emoji: string; };
...
const vehicle = computed(() => playQueue.value[currentVehicleIdx.value]);
...
function generateLevel() {
  ...
  log.generate({ level: level.value, vehicleId: vehicle.value.id, phase, pieces: rows.value * cols.value });
  safeSetTimeout(() => {
    const text = `Put the ${vehicle.value.label} together!`;
    log.audio(text);
...
function nextVehicle() {
  currentVehicleIdx.value++;
  if (currentVehicleIdx.value >= playQueue.value.length) {
    // Reshuffle when we run out
    playQueue.value = shuffleArray([...VEHICLES]);
    currentVehicleIdx.value = 0;
  }
  level.value++;
  completed.value = false;
  generateLevel();
...
<template>
  ...
  >
    <div v-if="!hasStarted" class="start-screen">
      <div class="start-icon"><MenuIcon gameId="fun-vehicle-jigsaw" style="width: 140px; height: 140px;" /></div>
      <h1>Vehicle Jigsaw<br><span>Spatial Puzzles</span></h1>
      <p class="start-sub">Put the vehicle pieces together.</p>
      ...
      <div class="action-layout">
        ...
        <header class="top-bar">
          <div class="phase-badge">
            <span style="font-size: 1.5rem; margin-right: 0.5rem">{{ vehicle.emoji }}</span>
            Level {{ level }} — {{ vehicle.label }}
          </div>
        ...
        >
          ...
          >
            <g v-if="isComplexScene">
              ...
              <g transform="translate(-10, 10) scale(0.6)"><ColorfulVehicle :name="playQueue[0]?.id" width="100" height="100" /></g>
              <g transform="translate(45, -5) scale(0.55)"><ColorfulVehicle :name="playQueue[1]?.id" width="100" height="100" /></g>
              <g transform="translate(25, 45) scale(0.7)"><ColorfulVehicle :name="playQueue[2]?.id" width="100" height="100" /></g>
              <g transform="translate(-20, 60) scale(0.4)"><ColorfulVehicle :name="playQueue[3]?.id" width="100" height="100" /></g>
              <g transform="translate(65, 55) scale(0.5)"><ColorfulVehicle :name="playQueue[4]?.id" width="100" height="100" /></g>
            </g>
            <ColorfulVehicle v-else :name="vehicle.id" width="100" height="100" />
          </svg>
    ...
    <transition name="pop">
      <div v-if="completed" class="complete-overlay">
        <div class="complete-card">
          <div class="complete-emoji">{{ vehicle.emoji }}</div>
          <h2 class="complete-title">{{ vehicle.label }}!</h2>
          <p class="complete-sub">Great job! You built it! 🎉</p>
          <div class="complete-actions">
            <button class="btn-next" @click="nextVehicle">Next Vehicle →</button>
